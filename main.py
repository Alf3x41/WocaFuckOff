...
def zobraziť_chybu(exit_code=None):
    ...
    problémy = []
    if not visual_cpp_nainštalované():
        problémy.append("Microsoft Visual C++ Redistributable")
    chýbajúce = chýbajúce_závislosti()
    if chýbajúce:
        problémy.append("Chýbajúce Python balíky: " + ", ".join(chýbajúce))
    správa = "WocaFuckOff sa nepodarilo pripraviť.\n\n"
    if problémy:
        správa += "Problém:\n- " + "\n- ".join(problémy) + "\n\n"
    if exit_code is not None:
        správa += f"Kód inštalátora: {exit_code}\n\n"
    správa += "Skontroluj inštaláciu a skús aplikáciu spustiť znova."
    messagebox.showerror("WocaFuckOff – Chyba", správa)
    root.destroy()
