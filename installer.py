...
def stiahnuť_visual_cpp():
    ...
    cieľ = (
        Path(tempfile.gettempdir())
        / "WocaFuckOff_vc_redist.x64.exe"
    )

    aktualizovať_stav(
        "Sťahujem Microsoft Visual C++ Runtime..."
    )
...
def nainštalovať_visual_cpp():
    ...
    if visual_cpp_nainštalované():
        ...
        aktualizovať_stav(
            "Microsoft Visual C++ je pripravený."
        )
    ...
    try:
        ...
        aktualizovať_stav(
            "Inštalujem Microsoft Visual C++ Runtime..."
        )
...
def nainštalovať_playwright():
    ...
    if výsledok.returncode != 0:
        ...
        raise RuntimeError(
            "Playwright sa nepodarilo pripraviť."
        )
...
def zobraziť_chybu(chyba):
    ...
    stav_label.config(
        text="Inštaláciu sa nepodarilo dokončiť."
    )
...
obrazovka_šírka = (
    okno.winfo_screenwidth()
)

obrazovka_výška = (
    okno.winfo_screenheight()
)
