...
try:
    ...
    print(
        f"CHYBA: Nepodarilo sa načítať config.toml: {chyba}",
        flush=True,
    )
    config = {}
...
DEBUG_PORT = config.get(
    "debug_port",
    "http://localhost:9222",
)

WORDLIST_FILE = config.get(
    "wordlist_file",
    "wordlist.json",
)

PICTURE_FILE = config.get(
    "picture_file",
    "picturelist.json",
)

PLACEHOLDER_WORDS = config.get(
    "placeholder_words",
    ["", "translate", "check"],
)

CLASS_INDEX = int(
    config.get("class_index", 0)
)

PACKAGE_INDEX = int(
    config.get("package_index", 0)
)
...
DOUBLE_POINTS = bool(
    config.get("double_points", False)
)

ADDON_POINTS = int(
    config.get("addon_points", 1000)
)

LAST_POINTS = int(
    config.get("last_points", 0)
)

AUTO_TRANSLATE = bool(
    config.get("auto_translate", True)
)
...
DEEPL_API_KEY = (
    ...
    or config.get("deepl_api_key", "")
).strip()

# DeepL API Developer / Pro
...
def nacitaj_slovnik():
    ...
    if not cesta.exists():
        try:
            ...
            log(
                f"Nepodarilo sa vytvoriť slovník: {chyba}"
            )
    ...
    try:
        ...
        log(
            f"Nepodarilo sa načítať slovník: {chyba}"
        )
...
def uloz_slovnik():
    ...
    try:
        ...
        log(
            f"Nepodarilo sa uložiť slovník: {chyba}"
        )
...
def nacitaj_obrazkovy_slovnik():
    ...
    try:
        ...
        log(
            f"Nepodarilo sa načítať obrázkový slovník: "
            f"{chyba}"
        )
...
def uloz_obrazkovy_slovnik():
    ...
    try:
        ...
        log(
            f"Nepodarilo sa uložiť obrázkový slovník: "
            f"{chyba}"
        )
...
def preloz_cez_deepl(text, cielovy_jazyk):
    ...
    if not DEEPL_API_KEY:
        log(
            "DeepL API kľúč nie je nastavený."
        )
    ...
    try:
        ...
        if not preklad_je_platny(
            text,
            prelozeny,
        ):
            log(
                f"DeepL vrátil neplatný preklad: "
                f"{text!r} -> {prelozeny!r}"
            )
...
def automaticky_preloz_slovo(slovo):
    ...
    preklad, zdrojovy_jazyk = preloz_cez_deepl(
        slovo,
        "EN",
    )
    ...
    log(
        f"DeepL nedokázal určiť smer prekladu "
        f"pre: {slovo!r}"
    )
...
def get_answer_auto_update(slovo):
    ...
    for otazka, odpoved in SLOVNIK.items():
        ...
        casti_otazky = [
            normalize(cast)
            for cast in str(otazka).split(",")
        ]

        casti_odpovede = [
            normalize(cast)
            for cast in str(odpoved).split(",")
        ]
    ...
    if AUTO_TRANSLATE:
        preklad = automaticky_preloz_slovo(
            slovo
        )
...
def posli_ntfy(sprava):
    ...
    try:
        ...
        if NTFY_TOKEN:
            hlavicky["Authorization"] = (
                f"Bearer {NTFY_TOKEN}"
            )

        requests.post(
            url,
            data=str(sprava).encode("utf-8"),
            headers=hlavicky,
            timeout=10,
        )
...
def spracuj_vyber_obrazka(stranka):
    try:
        ...
        if (
            "vyber obrázok" not in dolny_text
            and "choose picture" not in dolny_text
        ):
            return False
        ...
        for selektor in [
            ...
            try:
                prvok = stranka.locator(
                    selektor
                ).first

                if prvok.count() > 0:
                    otazka = prvok.inner_text(
                        timeout=500
                    ).strip()
        ...
        odpoved = get_answer_auto_update(
            otazka
        )
...
def spracuj_popis_obrazka(stranka):
    try:
        ...
        alt = obrazok.get_attribute(
            "alt"
        ) or ""

        title = obrazok.get_attribute(
            "title"
        ) or ""
...
def spracuj_padajuce_slovo(stranka):
    try:
        ...
        if not odpoved:
            log(
                f"Pre slovo '{slovo}' "
                f"nebola nájdená odpoveď."
            )

            return False

        vstup = stranka.locator(
        ...
        odoslat = stranka.locator(
            "#translateFallingWordSubmitBtn"
        ).first
...
def spracuj_vyber_slova(stranka):
    try:
        ...
        if (
            "vyber slovo" not in dolny_text
            and "choose word" not in dolny_text
        ):
            return False

        otazka_prvok = stranka.locator(
            "#q_word"
        ).first
        ...
        otazka = otazka_prvok.inner_text(
            timeout=500
        ).strip()
...
def spracuj_jedno_z_viacerych(stranka):
    try:
        ...
        if (
            "one out of many" not in dolny_text
            and "jedno z" not in dolny_text
        ):
            return False
...
def pripoj_prehladavac(playwright):
    ...
    try:
        log(
            f"Pripájam sa k prehliadaču cez "
            f"{DEBUG_PORT}..."
        )
        ...
        log(
            f"Nepodarilo sa pripojiť "
            f"k prehliadaču: {chyba}"
        )
...
def zabezpec_stranku(stranka):
    try:
        ...
        log(
            f"Nepodarilo sa načítať WocaBee: "
            f"{chyba}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="WocaFuckOff riešič"
    )
    ...
    DEBUG_PORT = argumenty.debug_port
    ...
    log(
        f"Adresa WocaBee: {URLBASE}"
    )

    log(
        f"Súbor slovníka: {WORDLIST_FILE}"
    )

    log(
        f"Počet uložených slov: "
        f"{len(SLOVNIK)}"
    )
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            if not zabezpec_stranku(
                stranka
            ):
                return 1

            log(
                "Riešič je pripravený."
            )
            ...
            log("")
            log(
                "Riešič bol zastavený používateľom."
            )
